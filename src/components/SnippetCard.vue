...
import { Copy, Check, Edit3, Trash2, Image as ImageIcon, FileText, ChevronUp, ChevronDown, List } from 'lucide-vue-next'
...
function previewText(content: string): string {
  ...
  return lines.slice(0, props.isExpanded ? 10 : 4).join('\n')
}
</script>

<template>
  <div
    class="snippet-card bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-4 cursor-pointer transition-all duration-300 hover:shadow-lg hover:border-indigo-300 dark:hover:border-indigo-600 group relative overflow-hidden flex flex-col"
    :class="[
      isExpanded ? 'h-auto row-span-2 col-span-2' : 'h-64',
      isExpanded ? 'z-10 shadow-xl ring-2 ring-indigo-500' : ''
    ]"
    @click="emit('expand')"
  >
    <div
    ...
    <div class="pl-2 flex-1 flex flex-col min-h-0">
      <div v-if="snippet.type === 'image' && snippet.imageUrl" class="mb-2 flex-shrink-0">
        <img
          ...
          :class="isExpanded ? 'h-48' : 'h-24'"
          @click.stop="handleCopyImage"
        />
      </div>

      <div v-else-if="snippet.type === 'multi' && snippet.items && snippet.items.length > 0" class="space-y-2 mb-2 flex-1 overflow-auto">
        <div
          v-for="(item, idx) in snippet.items.slice(0, isExpanded ? snippet.items.length : 4)"
          :key="item.id"
          class="flex items-center gap-2 p-2 rounded-md hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors group/item"
        ...
        >
          <img
            ...
            :class="isExpanded ? 'w-12 h-12' : 'w-8 h-8'"
            object-fit="cover"
          />
        ...
        <p v-if="snippet.items.length > 4 && !isExpanded" class="text-xs text-slate-400 text-center pt-1">
          还有 {{ snippet.items.length - 4 }} 项，点击卡片放大查看
        </p>
      </div>
